...
$$
\begin{aligned}
S_{R} * (n - C_{R}) + (S - S_{R}) * C_{R} \\
n * S_{R} + S * C_{R} - 2 * S_{R} * C_{R}
\end{aligned}
$$
...
Simple idea that I got from the very beginning. Overcomplicated the implementation which lead to a faster solution but late submission costing rank.

## FARSWAP

The idea extends from the previous problem. When we are on index $i$, the only thing we care about is whether the number goes after the $(i-1)$-th number or before which can be derived from the order in the input permutation.

So there is a DP solution to this with the states being:

1. The number from the permutation $1-N$ we are at
2. The position where the number goes

$dp[i][j]$ = Number of ways in which the $i$-th number from the permutation goes to index $j$

Then the transitions become:

1. If $(i - 1)$ appears before $i$ in the input permutation, then $dp[i][j] = dp[i - 1][0] + dp[i - 1][1] + ... dp[i - 1][j - 1]$
2. If $(i - 1)$ appears after $i$ in the input permutation, then $dp[i][j] = dp[i - 1][j]$ (when the $i$-th number is added to the permutation, the $(i-1)$-th number shifts from the $j$-th position) $+ dp[i - 1][j + 1] + ... dp[i - 1][i - 1]$
